...
<body class="back">
	...
	<div class="container container1 mt-5 bg-gradient">
		...
		<table
			...
			<tr>
			<% List<Task> list=(ArrayList<Task>)hs.getAttribute("allDone");
              if(list==null){
  ...
  <%    }
    	 ...
    	 for(Task item:list){
    		 java.util.Date date=new java.util.Date();
    			...
    			<td> <a href="done?tid=<%=item.getTaskId()%>&tname=<%=item.getTaskName()%>&tdesc=<%=item.getTaskDesc() %>"><button class="btn btn-danger">Delete</button></a></td>	
    			</tr>
    			<%    }
     }
	%>
